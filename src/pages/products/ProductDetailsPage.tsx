...
export function ProductDetailPage() {
  ...
  const product = productsList.find(item => item.id === Number(id ?? 1)) || productsList[0];
  ...
  const cartItem = cartItems.find(
    (i) => Number(i.productId) === product.id
  );
    ...
    <div className="relative flex min-h-screen flex-col bg-white">
          ...
          {[0, 1, 2].map((i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all ${
                i === activeDot ? 'w-5 bg-[#53b175]' : 'w-1.5 bg-gray-300'
              }`}
            />
          ))}
        </div>
      ...
      <div className="flex flex-1 flex-col px-5 pt-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-head text-left text-gray-900">{product.name}</h1>
          ...
          >
            ...
            />
              ...
              <div>
            <h1 className="text-subheading text-left text-gray-700">{product.desc}</h1>
            {/* <p className="mt-1 text-sm text-gray-400">{product.unit}</p> */}
          </div>
...
<div className="mt-5 flex items-center justify-between">
  <div className="flex items-center gap-3">
    ...
    <div className="flex h-9 w-12 items-center justify-center rounded-[17px] border border-gray-200 bg-white">
      <span className="text-sm font-semibold text-gray-900">{localQty}</span>
    </div>
    ...
    <button
      ...
      className="flex h-9 w-9 items-center justify-center transition hover:bg-gray-50"
    >
      <Plus className="h-3.5 w-3.5 text-[#53b175]" />
    </button>
  </div>
  ...
  <span className="text-xl font-bold text-gray-900">
    ${(Number(product.price.replace('$', '')) * localQty).toFixed(2)}
        ...
        >
          <span className="text-base font-semibold text-gray-900">Product Detail</span>
          <ChevronRight
        ...
        <button className="flex w-full items-center justify-between py-1">
          <span className="text-base font-semibold text-gray-900">Nutritions</span>
        ...
        <button className="flex w-full items-center justify-between py-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex">
              {[1, 2, 3, 4, 5].map((s) => (
                <span key={s} className={`text-sm ${s <= 4 ? 'text-yellow-400' : 'text-gray-200'}`}>
                  ★
                </span>
              ))}
            </div>
